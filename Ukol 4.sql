...
CREATE OR REPLACE VIEW ms_avgprice_year AS
SELECT
...
CREATE OR REPLACE VIEW ms_avgpayroll_year AS
SELECT 
...
CREATE OR REPLACE VIEW ms_avgprice_avgpayroll_year AS
SELECT 
	ms.price_year AS price_year, 
	ms.avg_price AS avg_price, 
	ms2.avg_payroll AS avg_payroll
FROM ms_avgprice_year ms 
LEFT JOIN ms_avgpayroll_year ms2 ON ms.price_year = ms2.payroll_year;
